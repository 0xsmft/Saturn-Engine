...
static constexpr unsigned int DT_IN_PROGRESS = 1u << 29;		// Operation still in progress.

// Detail information for status.
static constexpr unsigned int DT_STATUS_DETAIL_MASK = 0x0ffffff;
static constexpr unsigned int DT_WRONG_MAGIC = 1 << 0;		// Input data is not recognized.
static constexpr unsigned int DT_WRONG_VERSION = 1 << 1;	// Input data is in wrong version.
static constexpr unsigned int DT_OUT_OF_MEMORY = 1 << 2;	// Operation ran out of memory.
static constexpr unsigned int DT_INVALID_PARAM = 1 << 3;	// An input parameter was invalid.
static constexpr unsigned int DT_BUFFER_TOO_SMALL = 1 << 4;	// Result buffer for the query was too small to store all results.
